...
# Manual plugin folder: LMS never deletes it. InstalledPlugins/ is owned by the
# extension manager, which removes any folder not listed in extensions.prefs on startup.
DEST="/Users/musicplayer/Library/Application Support/Squeezebox/Plugins/SACDPlayer"   # absolute: no ~ expansion games with the space
...
# rsync 3.4 (local) vs 2.6.9 (server) disagree on how to quote a remote path with a space,
# so stage on the server in a space-free dir and let the server's own rsync do the last hop.
RSTAGE="/Users/musicplayer/.sacdplayer-deploy"
rsync -rlt --delete "$STAGE/" "$HOST:$RSTAGE/" || rsync -rt --delete "$STAGE/" "$HOST:$RSTAGE/"
ssh "$HOST" "rsync -rlt --delete \"$RSTAGE/\" \"$DEST/\" && rm -rf \"$RSTAGE\""
